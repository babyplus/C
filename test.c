...
void main(void)
{
	...
	DELIVER(delay,                       0, goodProgress, 0, "计算时延")
	DELIVER(delay_pro,                   0, goodProgress, 0, "计算时延")
	DELIVER(generate_random_32bit,       0, goodProgress, 0, "生成随机8bit")
	DELIVER(generate_hex_str,            0, goodProgress, 0, "随机生成16进制字符串")
	DELIVER(str_combine,                 0, goodProgress, 0, "字符串拼接")
	DELIVER(random_ip,                   1, goodProgress, 1, "通过时间戳随机生成5个127网段的ip")
	PRINT_PURPLE("===============================================================================\n")
	PRINT_SEPARATOR
	...
	DELIVER(receive_oam_pdu,             0, deprecated,   0, "使用recv接收enp0s8 CFM报文并处理,存在问题,会阻塞住程序")
	DELIVER(receive_oam_pdu_by_while,    0, betaAvailable,0, "使用recvmsg接收enp0s8 CFM报文并处理,存在问题,会阻塞住程序")
	DELIVER(receive_oam_pdu_by_select,   0, goodProgress, 0, "enp0s8接收CFM报文并处理")
	DELIVER(capture_oam_pdu,             0, started,      1, "捕获CFM报文并处理")
	PRINT_PURPLE("===============================================================================\n")
